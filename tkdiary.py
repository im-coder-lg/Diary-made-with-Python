...
lbl = Label(root, text="Hello! Write here:", font=("Arial Bold", 25),bg='lime') #Write text
lbl.grid(column=1, row=0)   #Places the text into the GUI

txt = Entry(root,width=30, bd =5)    #Creates textbox to take input from user
txt.grid(column=1, row=1)
  #Places the input box into the GUI
...
def createnewdiary():
    ...
    creatediary_syscommand = "notepad.exe" + new_diary_name2
...
def my_script1(): #template to include your own script
    ...
    my_syscommand = "python write2diary_withoutdate.py"  #or "python my_script1.py --input " + my_input
    os.system( my_syscommand ) 

#Responsive (but currently useless) button within GUI that calls the thanks() function defined above
btn1 = Button(root, text="Record Text", bg="orange", command=thanks)  #adds orange button that says click me, defines btn variable as this button 
btn1.grid(column=1, row=2)   #Places the button into the GUI

btn2 = Button(root, text="Write to My Diary", bg="blue", command=diarywrite)  #adds blue button that says "Write to Diary" and appends your text to my_diary.txt using the diarywrite() function defined above 
btn2.grid(column=1, row=3)   #Places the button into the GUI

btn3 = Button(root, text="Open Diary", bg="yellow", command=diaryopen)  #adds yellow button that says "Open Diary" and opens my_diary.txt using the diaryopen() function defined above 
btn3.grid(column=1, row=4)   #Places the button into the GUI

Exit = Button(root, text="Close", bg="red", command=qExit)  #closes the app
Exit.grid(column=1, row=5)

#Button to create a new diary: 
#btn4 = Button(root, text="Create New Diary", bg="green", command=createnewdiary)   
#btn4.grid(column=1, row=6)   #Places the button into the GUI

#Template button for your custom script 
#my_btn = Button(root, text="Write to diary without date", bg="red", command=my_script1)  #Runs your custom script
#my_btn.grid(column=1, row=7)   #Places the button into the GUI
